...
У циљу смањења мрежног оптерећења (енгл. _network overhead_) и кашњења, комуникација између клијента и сервера у овој апликацији је реализована коришћењем бинарне серијализације података уместо текстуалних формата као што је JSON. Иако је JSON читљив и једноставан за имплементацију, он је за потребе мрежмне синхронизације у реалном времену неефикасан из следећих разлога:
