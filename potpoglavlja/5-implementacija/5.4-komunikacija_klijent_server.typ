...
UDP комуникација (_PacketPeerUDP_): Како је приказано у методи _connect_to_socket()_, клијент врши резолуцију хоста преко _IP.resolve_hostname()_ како би динамички претворио доменско име или локалну адресу у валидну IP адресу. Коришћењем класе *_PacketPeerUDP_* омогућено је слање сирових бајтова (_PackedByteArray_) уз минимално кашњење, што је критично за игре са брзом синхронизацијом позиција играча и стања физичке симулације. Метода _send_data()_ врши безбедну проверу активне конекције пре саме серијализације и слања пакета ка серверу на дефинисаном порту (у овом случају 9000).

_WebSocket_ комуникација: Поред UDP-а, синглтон интегрише и *_WebSocketPeer_* механизам (дефинисан кроз променљиве попут: _websocket_, _is_connected_to_websocket_ и праћење губитка везе _is_connection_with_websocket_lost_). Ово омогућава поуздан, двосмерни комуникацијски канал преко TCP-а за догађаје који захтевају гарантовану испоруку порука, а који нису временски критични као сама физичка симулација.

Са друге стране, *_MyHttpHandler_* синглтон (приказан на листингу @lst:deo_MyHttpHandler_singltona) се ослања на  REST API комуникацију. Архитектура овог синглтона се заснива на асинхроном моделу захтев-одговор (_Request-Response_), где свака акциона метода има свој пратећи повратни позив (енгл. _callback_ методу) која се окида по завршетку HTTP захтева (_register()_ - _on_register_completed_(), _login()_ - _on_login_completed()_). Такође, иако је *_Network_* задужен за успостављање конекције, *_MyHttpHandler_* синглтон је задужен и за слање података ка серверу путем _webSocket_-а.
...
У циљу смањења мрежног оптерећења (енгл. _network overhead_) и кашњења, комуникација између клијента и сервера у овој апликацији је реализована коришћењем бинарне серијализације података уместо текстуалних формата као што је JSON. Иако је JSON читљив и једноставан за имплементацију, он је за потребе мрежне синхронизације у реалном времену неефикасан из следећих разлога:
...
- _boolean_ поља (u8 по пољу): 4 бајта,
