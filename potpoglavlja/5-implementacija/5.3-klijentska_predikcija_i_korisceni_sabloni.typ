...
Како би се избегле сталне *_if_* провере у _physics_process_() методи која се позива сваког фрејма, што, ако их има пуно, може да доведе до пада у перформансама, имплементиран је *_State_* шаблон представљен *_PlayerState_* апстрактном класом (видети листинг @lst:implementacija_PlayerState_klase). _Enter()_ и _update()_ методе морају бити редефинисане у конкретним имплементацијама класа, док су _apply_common_physics()_ и _handle_inputs()_ заједничке методе за сва стања. Постојеће класе стања су: *_IdleState_*, *_RunningState_*, *_JumpingState_* и *_DeadState_*, а њихове имплементације _enter()_ методе су приказане у листинзима @lst:IdleState_enter_metoda, @lst:RunningState_enter_metoda, @lst:JumpingState_enter_metoda и @lst:DeadState_enter_metoda. Играч садржи референцу на стање, а када је потребно да играч пређе у ново стање, потребно је позвати методу _change_player_state()_ (приказано на листингу @lst:change_state_metoda).
\
...
Постоје мале разлике које увек доводе до малог одступања између позиција на клијентској и серверској страни, без обзира како се кретање рачуна:
  ...
  - Чак и уз примену _tick-rate_ синхронизације, где је одређено да се физика рачуна 60 пута у секунди, због начина на који _Godot Game Engine_ и _Rapier_ заокружују децималне вредности, кроз велики број итерација може доћи до благог одступања у срачунатим позицијама (до неколико пиксела кроз одређени период).
